...
async def request_discount(campaign_id: str, body: RequestDiscountBody, user=Depends(require_roles(["sales", "admin"]))):
    ...
    if doc.to_dict().get("status") not in ["draft", "campaignConfigured", "discountRejected"]:
        raise HTTPException(status_code=400, detail="Discount can only be requested on a draft, configured, or rejected campaign")
...
async def review_discount(campaign_id: str, body: ReviewDiscountBody, user=Depends(require_roles(["adManager", "admin"]))):
    ...
    if not doc.exists:
        raise HTTPException(status_code=404, detail="Campaign not found")
    now = datetime.now(timezone.utc).isoformat()
    new_status = "discountApproved" if body.action == "approve" else "discountRejected"
    discount_status = "approved" if body.action == "approve" else "rejected"
    ref.update({
        "status": new_status, "discount.status": discount_status,
        "discount.reviewedBy": user["uid"], "discount.reviewedAt": now,
        "discount.approvedBy": user["uid"] if body.action == "approve" else None,
        "discount.approvedAt": now if body.action == "approve" else None,
        "updatedAt": now
    })
